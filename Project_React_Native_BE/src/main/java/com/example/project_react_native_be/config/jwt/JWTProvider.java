package com.example.project_react_native_be.config.jwt;
    ...
    public boolean validateRefreshToken(String token) {
        try {
            Claims claims = Jwts.parserBuilder()
                    ...
                    .parseClaimsJws(token)
                    .getBody();
            // Kiểm tra token có phải là refresh token không (thời gian hết hạn dài hơn)
            Date expiration = claims.getExpiration();
            Date issuedAt = claims.getIssuedAt();
            long tokenDuration = expiration.getTime() - issuedAt.getTime();
            // Refresh token phải có thời gian >= 7 ngày
            return tokenDuration >= (jwtRefresh - 1000); // Trừ 1 giây để tránh lỗi làm tròn
        } catch (ExpiredJwtException e) {
            log.error("Refresh token hết hạn!", e);
            return false;
        } catch (UnsupportedJwtException e) {
            log.error("Refresh token không được hỗ trợ!", e);
            return false;
        } catch (MalformedJwtException e) {
            log.error("Refresh token không đúng định dạng!", e);
            return false;
        } catch (SignatureException e) {
            log.error("Lỗi chữ ký refresh token!", e);
            return false;
        } catch (IllegalArgumentException e) {
            log.error("Lỗi tham số refresh token!", e);
            return false;
        }
    }
